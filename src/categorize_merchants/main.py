...
def categorize_merchants(request):
    df = pandas_gbq.read_gbq(query_or_table=MERCHANTS_TABLE_BQ, project_id=PROJECT_ID)
    df = df.head()

    gemini_api_key = read_gcp_secret(project_id=PROJECT_ID, secret_id=GEMINI_API_KEY)
    ...
    for index, merchant_row in df.iterrows():
        ...
        merchant_category = client.models.generate_content(
